...
router.get('/callback', async (req, res) => {
  try {
    ...
    // Set up push notifications to receive calendar updates
    try {
      await googleCalendarService.setupPushNotifications(doctorId);
      log.info(`Push notifications enabled for doctor ${doctorId}`);
    } catch (pushError) {
      log.error('Failed to setup push notifications:', pushError);
      // Don't fail the whole connection if push notifications fail
    }
...
router.delete('/disconnect/:doctorId', verifyJWT, async (req, res) => {
  try {
    ...
    // Stop push notifications first (while we still have valid tokens)
    try {
      await googleCalendarService.stopPushNotifications(doctorId);
    } catch (stopError) {
      log.warn('Failed to stop push notifications (non-critical):', stopError.message);
      // Continue anyway
    }
    ...
    if (doctor.google_refresh_token) {
      try {
        ...
        const hasValidAccessToken = doctor.google_access_token && 
          doctor.google_token_expires_at && 
          new Date(doctor.google_token_expires_at) > new Date();
        ...
        if (hasValidAccessToken) {
          ...
          log.info('Google token revoked successfully using existing access token');
        } else {
          // Refresh to get a new access token, then revoke
          oauth2Client.setCredentials({
            refresh_token: doctor.google_refresh_token
          });
          ...
          await oauth2Client.revokeCredentials();
          log.info('Google token revoked successfully after refresh');
        }
      } catch (revokeError) {
        // If revocation fails, log but continue - the token will expire anyway
        // This is non-critical since we're clearing it from the database
        log.debug('Could not revoke Google token (non-critical, will be cleared from DB):', revokeError.message);
        // Continue anyway to clear from database
      }
    }

    // Clear calendar data from database (core fields)
    const { error: updateError } = await supa
      .from('doctors')
      .update({
    ...
    // Try to clear push notification fields (may not exist if migration hasn't run)
    try {
      const { error: pushUpdateError } = await supa
        .from('doctors')
        .update({
      ...
      if (pushUpdateError) {
        // If columns don't exist, log but don't fail - this is non-critical
        if (pushUpdateError.message?.includes('column') && pushUpdateError.message?.includes('schema cache')) {
          log.debug('Push notification columns not found (migration may not have run):', pushUpdateError.message);
        } else {
          throw pushUpdateError;
        }
      }
    } catch (pushError) {
      // Non-critical: push notification fields may not exist
      log.debug('Could not clear push notification fields (non-critical):', pushError.message);
    }
