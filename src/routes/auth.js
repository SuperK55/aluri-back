  ...
  try {
    ...
    if (location !== undefined) {
      if (location && (typeof location !== 'string' || !location.trim())) {
        return res.status(400).json({
          ok: false,
          error: 'Invalid location. Please provide a non-empty value.'
        });
      }
...
router.post('/auth/api-tokens', async (req, res) => {
  try {
    const authHeader = req.headers.authorization;
    ...
    const { data: newToken, error: tokenError } = await supa
      .from('api_tokens')
      .insert({
        owner_id: decoded.id,
        name: name.trim(),
    ...
    if (tokenError) {
      log.error('API token creation error:', tokenError);
      return res.status(500).json({
        ok: false,
    ...
    log.error('API token creation error:', error);
    res.status(500).json({
      ok: false,
      error: 'Internal server error'
    });
...
router.get('/auth/api-tokens', async (req, res) => {
  try {
    const authHeader = req.headers.authorization;
    ...
    const { data: tokens, error: tokensError } = await supa
      .from('api_tokens')
      .select('id, name, token_prefix, last_used_at, is_active, created_at')
      .eq('owner_id', decoded.id)
      .eq('is_active', true)
      .order('created_at', { ascending: false });

    if (tokensError) {
      log.error('API tokens fetch error:', tokensError);
      return res.status(500).json({
        ok: false,
    ...
    log.error('API tokens fetch error:', error);
    res.status(500).json({
      ok: false,
      error: 'Internal server error'
    });
...
router.delete('/auth/api-tokens/:id', async (req, res) => {
  try {
    const authHeader = req.headers.authorization;
    ...
    if (deleteError) {
      log.error('API token revoke error:', deleteError);
      return res.status(500).json({
        ok: false,
    ...
    log.error('API token revoke error:', error);
    res.status(500).json({
      ok: false,
      error: 'Internal server error'
    });
